...
import time
import picamera
from sensor.communication import xbee#ここは違う
import os
import linecache
...
def log(path, *data):
    ...
    rfd = path.rfind('/')
    path_backup = '/home/rsec/cactus2022/' + path[rfd:]
    ...
    try:
        for i in range(0,10):
            ...
            if utc != -1.0 and lat != -1.0:
                ...
                break
            time.sleep(1)
        return lat, lon, judge
    except KeyboardInterrupt:
        close_gps()
        print("\r\nKeyboard Intruppted, Serial Closed")

def run_gps_with_Ohshima(): 
    lat1,lon1,judge = location()
    xbee.str_trans(f'{lat1} {lon1}')
    grs80=pyproj.Geod(ellps='GRS80')
    azimuth,bkw_azimuth,distance=grs80.inv(lon1,lat1,lon2,lat2)
    return (lat1,lon1,azimuth,bkw_azimuth,distance,judge)
    ...
    while True:
        ...
        if maximum>5:
            ...
            while i:
                ...
                if propotion>0.4:
                    print("Goal detected!")
                    log(path, "Goal detected!", datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
                    SERVO(0,0)
                    while True:
                        SERVO(0,0)
                        time.sleep(1)
...
if __name__ == '__main__':
    now_time = datetime.datetime.now()
    filename = 'log' + now_time.strftime('%Y%m%d_%H%M%S') + '.csv'
    path = '/home/rsec/cactus2022/'+filename
    print("start")
    log(path, "start", datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
    SERVO(0,0)
    #着地判定
    land_detection()
    log(path, "land detected!", datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
    SERVO(300,300)
    time.sleep(5)
    SERVO(0,0)
    offsets = getofset()
    ...
    while True:
        ...
        log(path, "start PID", datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
        if GPS_data[4]>10:
            i=1
            while i:
                #GPSの情報取得
                ...
                #PID制御
                    ...
                    SERVO(0,0)
        #距離が一定以下なら画像処理に移行
        log(path, "finish PID", datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
        log(path, "start camera drive", datetime.datetime.now().strftime('%Y%m%d_%H%M%S'))
        camera_drive()